...
void showHelp() {
  std::cout << "MMLogger 完整示例应用\n"
            ...
            << "命令行参数:\n"
            << "  --appid=NAME           设置应用标识符\n"
            << "  --sinktype=TYPE        设置日志后端类型 (Stdout, GLog, "
               "OptimizedGLog)\n"
            << "  --console=BOOL         启用/禁用控制台输出 (true, false, "
               "默认:false)\n"
            << "  --toTerm=LEVEL         设置控制台日志级别 (verbose, debug, "
               "info, warn, error, fatal)\n"
            << "  --file=BOOL            启用/禁用文件日志 (true, false)\n"
            << "  --filepath=PATH        设置日志文件路径\n"
            << "  --toFile=LEVEL         设置文件日志级别 (verbose, debug, "
               "info, warn, error, fatal)\n"
            << "  --debugSwitch=BOOL     启用/禁用调试日志 (true, false)\n"
            << "  --demo-mode=MODE       演示模式 (basic, threads, "
               "rate-limited, stress-test, all)\n"
            << "\n"
            << "OptimizedGLog 特有参数:\n"
            ...
            << "\n"
            << "示例:\n"
            << "  ./complete_example --sinktype=OptimizedGLog --console=true "
               "--toTerm=info --file=true --filepath=./logs\n"
            << "  ./complete_example --sinktype=GLog --file=true "
...
void showCurrentConfig(const mm::LogConfig& config) {
  ...
  switch (config.logSinkType_) {
    ...
    case mm::detail::LogSinkType_OptimizedGLog:
      std::cout << "OptimizedGLog";
      break;
...
void demoBasicLogging() {
  ...
  // 带格式化的日志
  MM_INFO("支持格式化: 整数=%d, 字符串=%s, 浮点数=%.2f", 42, "Hello World",
      3.14159);
...
void demoThreadedLogging() {
  ...
  MM_INFO("多线程日志演示开始 (%d线程, 每个线程%d条日志)", numThreads,
      logsPerThread);
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "\n示例完成。要查看完整的命令行选项，请使用 --help 参数运行。"
            << std::endl;
