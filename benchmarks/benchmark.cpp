...
public:
    PerformanceMonitor(const std::string& path = "/proc/self/io", const std::string& monPath = "") 
        : statFilePath(path), startDiskWrite(0), endDiskWrite(0), diskFd(-1), monitoredPath(monPath) {}
    ...
    bool start() {
        ...
        if (diskFd < 0) {
            std::cerr << "警告: 无法打开 " << statFilePath << " 获取磁盘I/O统计" << std::endl;
    ...
    double getCpuUsage() const {
        // 计算用户和系统CPU时间总和（秒）
        double startCpuTime = startUsage.ru_utime.tv_sec + startUsage.ru_stime.tv_sec +
                              (startUsage.ru_utime.tv_usec + startUsage.ru_stime.tv_usec) / 1e6;
        
        double endCpuTime = endUsage.ru_utime.tv_sec + endUsage.ru_stime.tv_sec +
    ...
    double getDirectorySize() const {
        ...
        try {
            for (const auto& entry : std::filesystem::recursive_directory_iterator(monitoredPath)) {
...
void logGenerationThread(int threadId, const BenchmarkConfig& config) {
    // 计算日志分布
    std::vector<double> logDistribution = {
    ...
    // 创建消息基础前缀
    std::string messagePrefix = "Thread " + std::to_string(threadId) + " - Log message ";
    ...
    const auto logIntervalNs = rateLimit ? 
        std::chrono::nanoseconds(1000000000 / (config.logRatePerSecond / config.numThreads)) : 
    ...
    if (config.useRateLimit) {
        int interval = (config.logRatePerSecond > 0) ? 
            (1000 / (config.logRatePerSecond / config.numThreads)) : 100;
        rateLimiter = new mm::detail::RateLimitedLog(std::chrono::milliseconds(interval));
    }
    
    // 主日志循环
    int logsGenerated = 0;
    std::mt19937 rng(std::random_device{}() + threadId);  // 线程特定的随机数生成器
    ...
    while (g_runBenchmark.load() && logsGenerated < config.logsPerThread) {
        ...
        if (config.measureLatency) {
            ...
            auto endTime = std::chrono::high_resolution_clock::now();
            auto latency = std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - startTime).count();
...
// 计算延迟百分位数
void calculateLatencyPercentiles(const std::vector<uint64_t>& latencies, 
...
std::string getSystemInfo() {
    ...
    if (statvfs("/", &diskStat) == 0) {
        ...
        info += "磁盘: 总计=" + std::to_string(totalSpace / (1024*1024*1024)) + 
                "GB, 可用=" + std::to_string(freeSpace / (1024*1024*1024)) + "GB";
    }
...
bool getLoggerInternalStats(PerformanceMetrics& metrics, 
    ...
    try {
        ...
        if (logFile.is_open()) {
            ...
            while (std::getline(logFile, line)) {
                if (line.find("OptimizedGlogLogger stats") != std::string::npos) {
                    // 示例行: "OptimizedGlogLogger stats - Enqueued: 15723, Processed: 15723, Dropped: 622990, Overflow: 81336"
                    // 解析这行获取数据
                    size_t pos;
        ...
        if (config.queueCapacity > 0) {
            // 估计队列利用率
            metrics.queueUtilization = std::min(
                1.0, metrics.logsPerSecond / (config.queueCapacity * 10.0)
            );
        }
...
void writeResultsToCSV(const BenchmarkConfig& config, 
    ...
    if (config.appendOutput) {
        ...
        if (outputFile.tellp() == 0) {
            outputFile << "Timestamp,TestID,TestName,LoggerType,ConsoleOutput,FileOutput,LogLevel,"
                      ...
                      << "AvgLatencyUs,P50LatencyUs,P90LatencyUs,P95LatencyUs,P99LatencyUs,MaxLatencyUs,"
                      << "CPUPercent,MemoryMB,DiskWritesBps,DiskWritesMBs,"
                      << "EnqueuedCount,ProcessedCount,DroppedCount,OverflowCount,QueueUtilization,"
                      << "SystemInfo"
                      << std::endl;
        }
    }
    ...
    outputFile 
        ...
        << config.logLevel << ","
        << config.numThreads << ","
        << config.logsPerThread << ","
        << config.logMessageSize << ","
        ...
        << config.numWorkers << ","
        << config.poolSize << ","
        << std::fixed << std::setprecision(2) << metrics.logsPerSecond << ","
        ...
        << std::fixed << std::setprecision(2) << metrics.cpuUsagePercent << ","
        << std::fixed << std::setprecision(2) << metrics.memoryUsageMB << ","
        << metrics.diskWritesBytesPerSec << ","
        << std::fixed << std::setprecision(2) << metrics.diskWritesMBPerSec << ","
...
bool parseArgs(int argc, char* argv[], BenchmarkConfig& config) {
    ...
    const char* envVars[] = {
        "TEST_ID", "TEST_NAME", "LOGGER_TYPE", "ENABLE_CONSOLE", "ENABLE_FILE", 
        "LOG_FILE_PATH", "LOG_LEVEL", "NUM_THREADS", "LOGS_PER_THREAD", 
        "LOG_MSG_SIZE", "LOG_RATE", "DEBUG_PCT", "INFO_PCT", "WARN_PCT", 
        "ERROR_PCT", "BATCH_SIZE", "QUEUE_CAPACITY", "NUM_WORKERS", "POOL_SIZE", 
        "WARMUP_SECONDS", "TEST_DURATION", "COOLDOWN_SECONDS", "OUTPUT_FILE", 
        "APPEND_OUTPUT", "VERBOSE_OUTPUT", "MEASURE_LATENCY", "USE_RATE_LIMIT"
    };
    ...
    // 设置默认值或使用环境变量值
    config.testId = envValues.count("TEST_ID") ? envValues["TEST_ID"] : "benchmark";
    config.testName = envValues.count("TEST_NAME") ? envValues["TEST_NAME"] : "MMLogger性能测试";
    config.loggerType = envValues.count("LOGGER_TYPE") ? envValues["LOGGER_TYPE"] : "OptimizedGLog";
    config.enableConsoleOutput = envValues.count("ENABLE_CONSOLE") ? (envValues["ENABLE_CONSOLE"] == "true") : true;
    config.enableFileOutput = envValues.count("ENABLE_FILE") ? (envValues["ENABLE_FILE"] == "true") : false;
    config.logFilePath = envValues.count("LOG_FILE_PATH") ? envValues["LOG_FILE_PATH"] : "./logs";
    config.logLevel = envValues.count("LOG_LEVEL") ? envValues["LOG_LEVEL"] : "info";
    
    config.numThreads = envValues.count("NUM_THREADS") ? std::stoi(envValues["NUM_THREADS"]) : 4;
    config.logsPerThread = envValues.count("LOGS_PER_THREAD") ? std::stoi(envValues["LOGS_PER_THREAD"]) : 100000;
    config.logMessageSize = envValues.count("LOG_MSG_SIZE") ? std::stoi(envValues["LOG_MSG_SIZE"]) : 128;
    config.logRatePerSecond = envValues.count("LOG_RATE") ? std::stoi(envValues["LOG_RATE"]) : 0;
    
    config.debugLogPercentage = envValues.count("DEBUG_PCT") ? std::stod(envValues["DEBUG_PCT"]) : 10.0;
    config.infoLogPercentage = envValues.count("INFO_PCT") ? std::stod(envValues["INFO_PCT"]) : 60.0;
    config.warnLogPercentage = envValues.count("WARN_PCT") ? std::stod(envValues["WARN_PCT"]) : 20.0;
    config.errorLogPercentage = envValues.count("ERROR_PCT") ? std::stod(envValues["ERROR_PCT"]) : 10.0;
    
    config.batchSize = envValues.count("BATCH_SIZE") ? std::stoi(envValues["BATCH_SIZE"]) : 200;
    config.queueCapacity = envValues.count("QUEUE_CAPACITY") ? std::stoi(envValues["QUEUE_CAPACITY"]) : 10000;
    config.numWorkers = envValues.count("NUM_WORKERS") ? std::stoi(envValues["NUM_WORKERS"]) : 4;
    config.poolSize = envValues.count("POOL_SIZE") ? std::stoi(envValues["POOL_SIZE"]) : 20000;
    
    config.warmupSeconds = envValues.count("WARMUP_SECONDS") ? std::stoi(envValues["WARMUP_SECONDS"]) : 2;
    config.testDurationSeconds = envValues.count("TEST_DURATION") ? std::stoi(envValues["TEST_DURATION"]) : 10;
    config.cooldownSeconds = envValues.count("COOLDOWN_SECONDS") ? std::stoi(envValues["COOLDOWN_SECONDS"]) : 2;
    
    config.outputFile = envValues.count("OUTPUT_FILE") ? envValues["OUTPUT_FILE"] : "benchmark_results.csv";
    config.appendOutput = envValues.count("APPEND_OUTPUT") ? (envValues["APPEND_OUTPUT"] == "true") : true;
    config.verboseOutput = envValues.count("VERBOSE_OUTPUT") ? (envValues["VERBOSE_OUTPUT"] == "true") : true;
    config.measureLatency = envValues.count("MEASURE_LATENCY") ? (envValues["MEASURE_LATENCY"] == "true") : true;
    config.useRateLimit = envValues.count("USE_RATE_LIMIT") ? (envValues["USE_RATE_LIMIT"] == "true") : false;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--help") {
            ...
            std::cout << "  --test-id=ID             测试ID (默认: " << config.testId << ")" << std::endl;
            std::cout << "  --test-name=NAME         测试名称 (默认: " << config.testName << ")" << std::endl;
            std::cout << "  --logger-type=TYPE       日志器类型: Stdout, GLog, OptimizedGLog (默认: " << config.loggerType << ")" << std::endl;
            std::cout << "  --enable-console=BOOL    是否启用控制台输出 (默认: " << (config.enableConsoleOutput ? "true" : "false") << ")" << std::endl;
            std::cout << "  --enable-file=BOOL       是否启用文件输出 (默认: " << (config.enableFileOutput ? "true" : "false") << ")" << std::endl;
            std::cout << "  --log-file-path=PATH     日志文件路径 (默认: " << config.logFilePath << ")" << std::endl;
            std::cout << "  --log-level=LEVEL        日志级别: debug, info, warn, error (默认: " << config.logLevel << ")" << std::endl;
            std::cout << "  --num-threads=N          日志线程数量 (默认: " << config.numThreads << ")" << std::endl;
            std::cout << "  --logs-per-thread=N      每线程日志数量 (默认: " << config.logsPerThread << ")" << std::endl;
            std::cout << "  --log-msg-size=N         日志消息大小(字节) (默认: " << config.logMessageSize << ")" << std::endl;
            std::cout << "  --log-rate=N             每秒日志数量(0表示最快速度) (默认: " << config.logRatePerSecond << ")" << std::endl;
            std::cout << "  --measure-latency=BOOL   是否测量延迟 (默认: " << (config.measureLatency ? "true" : "false") << ")" << std::endl;
            std::cout << "  --use-rate-limit=BOOL    是否使用速率限制 (默认: " << (config.useRateLimit ? "true" : "false") << ")" << std::endl;
            std::cout << "  --debug-pct=N            DEBUG级别日志百分比 (默认: " << config.debugLogPercentage << ")" << std::endl;
            std::cout << "  --info-pct=N             INFO级别日志百分比 (默认: " << config.infoLogPercentage << ")" << std::endl;
            std::cout << "  --warn-pct=N             WARN级别日志百分比 (默认: " << config.warnLogPercentage << ")" << std::endl;
            std::cout << "  --error-pct=N            ERROR级别日志百分比 (默认: " << config.errorLogPercentage << ")" << std::endl;
            std::cout << "  --batch-size=N           批处理大小 (默认: " << config.batchSize << ")" << std::endl;
            std::cout << "  --queue-capacity=N       队列容量 (默认: " << config.queueCapacity << ")" << std::endl;
            std::cout << "  --num-workers=N          工作线程数 (默认: " << config.numWorkers << ")" << std::endl;
            std::cout << "  --pool-size=N            内存池大小 (默认: " << config.poolSize << ")" << std::endl;
            std::cout << "  --warmup-seconds=N       预热时间(秒) (默认: " << config.warmupSeconds << ")" << std::endl;
            std::cout << "  --test-duration=N        测试持续时间(秒) (默认: " << config.testDurationSeconds << ")" << std::endl;
            std::cout << "  --cooldown-seconds=N     冷却时间(秒) (默认: " << config.cooldownSeconds << ")" << std::endl;
            std::cout << "  --output-file=PATH       输出文件路径 (默认: " << config.outputFile << ")" << std::endl;
            std::cout << "  --append-output=BOOL     是否追加到输出文件 (默认: " << (config.appendOutput ? "true" : "false") << ")" << std::endl;
            std::cout << "  --verbose=BOOL           是否显示详细输出 (默认: " << (config.verboseOutput ? "true" : "false") << ")" << std::endl;
            return false;
        }
        ...
        if (equalPos != std::string::npos && arg.substr(0, 2) == "--") {
            ...
            if (key == "test-id") config.testId = value;
            else if (key == "test-name") config.testName = value;
            else if (key == "logger-type") config.loggerType = value;
            else if (key == "enable-console") config.enableConsoleOutput = (value == "true");
            else if (key == "enable-file") config.enableFileOutput = (value == "true");
            else if (key == "log-file-path") config.logFilePath = value;
            else if (key == "log-level") config.logLevel = value;
            else if (key == "num-threads") config.numThreads = std::stoi(value);
            else if (key == "logs-per-thread") config.logsPerThread = std::stoi(value);
            else if (key == "log-msg-size") config.logMessageSize = std::stoi(value);
            else if (key == "log-rate") config.logRatePerSecond = std::stoi(value);
            else if (key == "measure-latency") config.measureLatency = (value == "true");
            else if (key == "use-rate-limit") config.useRateLimit = (value == "true");
            else if (key == "debug-pct") config.debugLogPercentage = std::stod(value);
            else if (key == "info-pct") config.infoLogPercentage = std::stod(value);
            else if (key == "warn-pct") config.warnLogPercentage = std::stod(value);
            else if (key == "error-pct") config.errorLogPercentage = std::stod(value);
            else if (key == "batch-size") config.batchSize = std::stoi(value);
            else if (key == "queue-capacity") config.queueCapacity = std::stoi(value);
            else if (key == "num-workers") config.numWorkers = std::stoi(value);
            else if (key == "pool-size") config.poolSize = std::stoi(value);
            else if (key == "warmup-seconds") config.warmupSeconds = std::stoi(value);
            else if (key == "test-duration") config.testDurationSeconds = std::stoi(value);
            else if (key == "cooldown-seconds") config.cooldownSeconds = std::stoi(value);
            else if (key == "output-file") config.outputFile = value;
            else if (key == "append-output") config.appendOutput = (value == "true");
            else if (key == "verbose") config.verboseOutput = (value == "true");
            else {
...
int main(int argc, char* argv[]) {
    ...
    if (config.verboseOutput) {
        ...
        std::cout << "  消息大小: " << config.logMessageSize << " 字节" << std::endl;
        std::cout << "  测试持续时间: " << config.testDurationSeconds << " 秒" << std::endl;
    }
    ...
    std::vector<std::string> loggerArgs = {
        "--sinktype=" + config.loggerType,
        "--toTerm=" + config.logLevel
    };
    ...
    if (config.loggerType == "OptimizedGLog") {
        ...
        loggerArgs.push_back("--queueCapacity=" + std::to_string(config.queueCapacity));
    ...
    if (config.warmupSeconds > 0) {
        if (config.verboseOutput) {
            std::cout << "预热阶段 (" << config.warmupSeconds << " 秒)..." << std::endl;
        }
    ...
    // 主测试阶段
    if (config.verboseOutput) {
        std::cout << "主测试阶段 (" << config.testDurationSeconds << " 秒)..." << std::endl;
    }
    ...
    // 计算指标
    metrics.logsPerSecond = static_cast<double>(g_totalLogsGenerated) / elapsedSeconds;
    ...
    metrics.diskWritesMBPerSec = metrics.diskWritesBytesPerSec / (1024.0 * 1024.0);
    ...
    if (config.cooldownSeconds > 0) {
        if (config.verboseOutput) {
            std::cout << "冷却阶段 (" << config.cooldownSeconds << " 秒)..." << std::endl;
        }
    ...
    if (config.verboseOutput) {
        ...
        std::cout << "线程数: " << config.numThreads << std::endl;
        std::cout << "\n--- 吞吐量指标 ---" << std::endl;
        std::cout << "日志吞吐量: " << std::fixed << std::setprecision(2) << metrics.logsPerSecond << " 日志/秒" << std::endl;
        std::cout << "数据吞吐量: " << std::fixed << std::setprecision(2) << metrics.bytesPerSecond / (1024.0 * 1024.0) << " MB/秒" << std::endl;
        
        std::cout << "\n--- 延迟指标 (微秒) ---" << std::endl;
        std::cout << "平均延迟: " << std::fixed << std::setprecision(2) << metrics.avgLatency << std::endl;
        std::cout << "P50延迟: " << std::fixed << std::setprecision(2) << metrics.p50Latency << std::endl;
        std::cout << "P90延迟: " << std::fixed << std::setprecision(2) << metrics.p90Latency << std::endl;
        std::cout << "P95延迟: " << std::fixed << std::setprecision(2) << metrics.p95Latency << std::endl;
        std::cout << "P99延迟: " << std::fixed << std::setprecision(2) << metrics.p99Latency << std::endl;
        std::cout << "最大延迟: " << std::fixed << std::setprecision(2) << metrics.maxLatency << std::endl;
        
        std::cout << "\n--- 资源使用 ---" << std::endl;
        std::cout << "CPU使用率: " << std::fixed << std::setprecision(2) << metrics.cpuUsagePercent << "%" << std::endl;
        std::cout << "内存使用: " << std::fixed << std::setprecision(2) << metrics.memoryUsageMB << " MB" << std::endl;
        std::cout << "磁盘写入: " << std::fixed << std::setprecision(2) << metrics.diskWritesMBPerSec << " MB/秒" << std::endl;
        ...
        if (config.loggerType == "OptimizedGLog") {
            ...
            std::cout << "队列利用率: " << std::fixed << std::setprecision(2) << (metrics.queueUtilization * 100.0) << "%" << std::endl;
        }
        
        std::cout << "\n--- 配置参数 ---" << std::endl;
        std::cout << "日志消息大小: " << config.logMessageSize << " 字节" << std::endl;
        std::cout << "日志速率限制: " << (config.logRatePerSecond > 0 ? std::to_string(config.logRatePerSecond) + " 日志/秒" : "无限制") << std::endl;
